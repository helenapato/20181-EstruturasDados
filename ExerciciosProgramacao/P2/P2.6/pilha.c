...
{
	nodep_t *visita = pilha->topo;
	int min = visita->valor;
	pilha_t *pilha_remover = cria_pilha();
	while(visita != NULL)
	{
		if(visita->valor < min) min = visita->valor;
		visita = visita->deBaixo;
	}
	visita = pilha->topo;
	while(visita != NULL)
	{
		...
		{
			push(pilha_remover, visita->valor);
			visita = visita->deBaixo;
			pop(pilha);
		}
	}
	pilha->topo = pilha_remover->topo;
/*	visita = pilha_remover->topo;
	while(visita != NULL)
	{
		push(pilha, visita->valor);
		visita = visita->deBaixo;
	}	*/
}
